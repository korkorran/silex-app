...
function Model({ name, ...props } : any) {
  ...
  const { nodes } : {nodes: any} = useGLTF('/compressed.glb')
...
function Controls() {
  ...
  return (
    <>
      ...
      {snap.current && <TransformControls object={scene.getObjectByName(snap.current)} mode={modes[snap.mode] as "translate" | "scale" | "rotate" | undefined } />}
...
function ThreeDobjects() {
  return (
    ...
    <p><b>Instructions : </b> Click on objects to position them. <a href='https://github.com/frederic-lang/awesome-konva-react-demos/blob/master/src/demos/3Dobjects' target='_blank'><FaGithub /></a></p>
    </>
  )
}
